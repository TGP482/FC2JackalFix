...
//
// Skipped for the mouse as well as for the setting: CActionMapPadFilter runs off the action map,
// which has forgotten what produced the action by the time these run, so with a pad plugged in the
// four helpers pull the mouse's aim about as well. The device flag watches the drivers themselves
// and answers what the player last touched.
