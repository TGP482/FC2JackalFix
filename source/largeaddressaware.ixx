...
// Enables large address awareness so Far Cry 2 can use more than 2 GB of address space.
static constexpr auto szTitle = L"FC2JackalFix";

// <exe>.bak is the executable as it was before the last change this made, in either direction.
...
// Validate before modifying and only support 32-bit PE files.
...
// The running image, which is what the 2 GB cap actually applies to.
...
static void SetChecksum(std::vector<uint8_t>& image)
{
    ...
    // The CheckSum field is treated as zero, so it is skipped during the calculation.
    ...
    // An odd sized file leaves a trailing byte the word loop never reaches.
    if (image.size() & 1)
        checksum += image.back();
...
{
    // The game's own executable is open as an image section, so share reads or this fails.
...
static bool SwapExecutable(const std::filesystem::path& exePath, const std::vector<uint8_t>& image)
{
    ...
    {
        // Put the original back rather than leave the game with no executable at all.
