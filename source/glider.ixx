/*
  Two hang glider fixes from An Almost Complete Guide to Far Cry 2 Modding: gliders shot out of the
  sky, and gliders bouncing on water. The guide edits three floats in every Air.Paraglider*
  prototype in entitylibrarypatchoverride.fcb:
  ...
  fMass, at CVehicleParagliderPhysComponent + 0x1B0. FUN_10492430 registers it at +0x90 of
  CPhysParagliderVehicleEntityCreateParams, and FUN_10073ED0 embeds that struct at +0x120. Its
  consumer FUN_104A15B0 is no use as a hook site: five physics-entity vtables reference it, and
  ...
  there beats setMass, the inertia tensor and Init's copy into the entity's slot at +0x190. The hook
  sits on MOV ESI,EAX / ADD ESP,8, five bytes exactly, where EBX already holds params and EAX still
  holds the allocation the MOV wants.

  fDiscardedMass, at CVehicleParagliderPhysComponent + 0x220. Registered by FUN_10073ED0, seeded to
  75.0 at 0x1006D678. Only reader is FUN_1006A230, slot +0x11C of the component vtable, called by
  the get-out-vehicle handler FUN_100E5B40 once the player is two metres away: FLD [ESI+0x220] into
  hkpRigidBody::setMass. 75kg is what lets an abandoned canopy skitter across water. The hook covers
  the six-byte FLD and writes the field first, so the FLD reads 825.

  fUnderWaterMaxDepth, at CVehicle + 0x114. Registered by FUN_100DEA10, seeded to -1.0 at
  0x100E11BE. Every vehicle in the game has it (the big truck ships 1.95), so it cannot be rewritten
  blind. Only reader is FUN_100DA270, CVehicle::IsSubmerged:
  ...
  -1 disables the test rather than always tripping it, so a stock paraglider is never recognised as
  submerged, never retired, and sits on the surface being pushed around by it. 1.5 arms the check.
  Callers FUN_100DB6E0, FUN_100DC930 and FUN_100E3450 are the vehicle entry and usability gates, so
  this runs for a nearby vehicle rather than every vehicle every frame.

  Identified by sName at CVehicle + 0: every Air.Paraglider* prototype carries the same name hash
  and no other vehicle does. iAnimVehicleType at +4 is 2 for the paraglider but is an animation set
  rather than an identity (the big truck's is 8).
*/
...
// CVehicle. sName is the hash of the vehicle's name string, "paraglider".
...
// The guide's tested values. Above 2420 gunfire stops registering at all; below it the glider
// still flips.
...
public:
    Glider()
    {
        JackalFix::onDuniaInitEvent() += []()
        {
            ...
            // vtable store at +0x47. Call displacements wildcarded.
            ...
            // Bouncing on water, half one: all of OnDiscarded, ending on the FLD the hook covers.
            ...
            // Bouncing on water, half two. IsSubmerged entry through the entity fetch, stopping
            // before the first call displacement. The hook covers the seven-byte COMISS, which
            // then reads the rewritten field.
