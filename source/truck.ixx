/*
  ...
  The guide's version is a data edit to entitylibrarypatchoverride.fcb: the Land.BigTruck* engine
  sounds point at ids that produce almost nothing, and the fix repoints the seven in SoundRemaps
  below across five prototypes (Land.BigTruck, .A2LM09_NitrousTruck, .ScriptedBigTruck, .Tanker and
  Land.BigTruck_Tanker), which needs a repacked patch.dat.

  They are indices 0, 1, 2, 4, 5, 6 and 10 of uint32_t m_SoundIds[21] at +0x2A0 of the vehicle
  engine and FX component (schema FUN_101AF0C0, constructor FUN_101ACFE0, vtable 0x10E266EC): every
  sound property registers against field offset 0x2A0 with its own array index. FUN_100F7420 is
  their only deserialiser (descriptor vtables 0x10E1C290 and 0x10E26670), sscanfing the archive's
  literal "0x0045CD73" text into object + desc->offset + desc->index * 4 at entity library load.
  Different function and vtable from the float serialiser renderconfig.ixx hooks, so the two do not
  collide.
  ...
  shipped vehicle library, so a value match covers all five whatever they inherit from. The 0x2A0
  field offset gate confines the remap to this class; the other three sound-name schemas use
  0x268/0x2D8, 0x224 and 0x58-0x6C/0x2C.
*/
...
public:
    Truck()
    {
        ...
        {
            // The sound-name deserialiser, entry to the parse call. No absolute addresses in that
            // span (0xD8 is the archive's ReadString vtable slot) and it stops before the call
            // displacement.
