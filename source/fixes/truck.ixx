...
// Sound-name property descriptor: six dwords, the array entry index sitting between offset and
// metadata.
...
// One entry per distinct id, so the guide's seven changes are six pairs.
...
// Runs after the loader writes the id; an omitted property stays 0xFFFFFFFF and matches nothing.
...
public:
    Truck()
    {
        ...
        {
            // Entry to the parse call; 0xD8 is the archive's ReadString vtable slot.
            ...
            {
                // Read once at startup, so no ini watch and no toggle.
