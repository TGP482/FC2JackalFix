...
// The {name, hash} pair every node accessor takes.
struct NameKey
{
    const char* pszName;
    uint32_t nHash;
...
// NameHash::Set, the engine's own hash, so hashes match the archive.
...
{
    // Singleplayer only, matching Boggalog; the .Multi variants are left alone.
...
static void ApplyToNode(Prototype ePrototype, FCBNode* pNode)
{
    ...
    case Prototype::M79:
        // fMoveSpeedFactor is 1.0 on both IronSight +0xD8 and weapon root +0xD4; bCanIronsight
        // exists only on the IronSight block.
    ...
    case Prototype::AssassinationTarget:
        // All three on the one FOVMultipliers node.
...
// Two-level walk, libraries then prototypes; pRoot is the wrapper.
...
public:
    EntityLibrary()
    {
        ...
        {
            // NameHash::Set, entered at the store of the computed hash.
            ...
            // Per-file prototype indexer, entered at its one-time init guard.
            ...
            // Rehashing a known hash validates NameHash before the others are trusted.
            uint32_t nCheck = 0;
