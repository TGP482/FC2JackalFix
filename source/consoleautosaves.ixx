/*
  ...
  The page builds the 360's box out of the MessagesBoxListSavePointCheck table: header
  MBOXHEADER_SAVEGAME, entries MBOXLISTSAVE_SAVEGAME and MBOXLISTSAVE_CONTINUENOSAVE. Its selection
  callback (0x108509A0) either pushes CSavePointSaveGamePage or fires GREventNextState. Neither page
  nor state tests for a save point, so all gating lives in whatever posts the event. The event name
  (0x10E95B34) is materialised at seven sites: CBedroll tick and OnMessage (0x106C184F, 0x106C1DA3),
  CPlayerSoundAndFXComponent::Update (0x106D3027, 0x106D3036), the SaveGame script command
  (0x1070E31C, 0x1070E32B) and the OnGREvent consumer (0x107F8958). None is a mission ending, so the
  360's behaviour came from its mission scripts calling SaveGame() in the packed data.

  Do not re-check the 360 executable: the string is at 0x820754DC, but nothing in the disassembled
  portion materialises it with a lis/addi pair, so its call sites cannot be enumerated.

  The trigger here is the on screen banner itself, one push through
  CFCXObjectiveHudManager::PushNewObjective (0x10686080) carrying an oasis section and text key.
  Every popup in the game funnels through it: COnScreenPopup replaying an OnScreenData.xml entry
  ...
  takes section, icon, text, soundEvent, soundType and time. Excluding the tutorial handoff by its
  Grin icon (21) also excludes every real underground mission, which shares both the icon and the
  A0GM00_00 objective.
*/
